...
def prepare_skinseg(dataset_id: int, task_name: str, test_size: float, n_folds: int, regenerate: bool = False):
    ...
    train_val, test, patient_metadata = get_train_test_split(df_labels, test_size, n_folds)
    df = pd.merge(
    left=df_labels, 
    ...
    print(f"Train images: {train_pids}, Test images: {test_pids}")
...
if __name__ == "__main__":
    # Example usage
    prepare_skinseg(dataset_id=1, task_name="SkinSegmentation", test_size= 0.25, n_folds = 4, regenerate = True)